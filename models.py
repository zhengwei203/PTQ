...
from qp_quant_ptq import Conv2dPTQ,LinearPTQ,Conv1dPTQ
...
class RepConv(nn.Module):
    def __init__(
        ...
        conv1x1 = Conv2dPTQ(in_channel, in_channel, 1, 1, 0, bias=False, groups=1)
        bn = BNAndPadLayer(pad_pixels=1, num_features=in_channel)
        conv3x3 = nn.Sequential(
            Conv2dPTQ(in_channel, in_channel, 3, 1, 0, groups=in_channel, bias=False),
            Conv2dPTQ(in_channel, out_channel, 1, 1, 0, groups=1, bias=False),
...
class SepConv(nn.Module):
    ...
    def __init__(
        ...
        self.pwconv1 = Conv2dPTQ(dim, med_channels, kernel_size=1, stride=1, bias=bias)
        self.bn1 = nn.BatchNorm2d(med_channels)
        self.lif2 = Multispike()
        self.dwconv = Conv2dPTQ(
            med_channels,
            med_channels,
        ...
        self.pwconv2 = Conv2dPTQ(med_channels, dim, kernel_size=1, stride=1, bias=bias)
...
class Spiking_vit_MetaFormer(nn.Module):
    def __init__(
        ...
        self.lif = Multispike()
        self.head = LinearPTQ(embed_dim[3], num_classes) if num_classes > 0 else nn.Identity()
...
def spikformer_8_15M_CAFormer(nb_classes=1000, **kwargs):
    print(f"Building model with {nb_classes} classes")
    model = Spiking_vit_MetaFormer(
        img_size_h=128,
        img_size_w=128,
        patch_size=16,
