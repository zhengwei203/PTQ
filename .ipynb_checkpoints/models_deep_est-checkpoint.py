...
class Spiking_vit_MetaFormer(nn.Module):
    def __init__(
        ...
        self.head = nn.Linear(embed_dim[3], 1) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        return x.reshape(T, B, C, H, W).permute(0, 1, 3, 4, 2).contiguous()
    ...
    def forward(self, x):
        ...
        # NOTE: 确保下一行的输入形状为 T B C H W 即可
        x = self.forward_features(x)

        # NOTE: 确保下一行的输入是 T B H W C
        # TODO: 修改 head
        x = self.head(self.lif(x)).mean(dim=0).squeeze(-1)
        # NOTE: head 应该是一个将 channels 缩为 1 的模块，我目前用了最简单的 nn.Linear(channels, 1)

        # TODO: 修改 upsample 模块
        x = F.interpolate(
            x.unsqueeze(1), size=(H, W), mode="bicubic", align_corners=False
        )
...
if __name__ == "__main__":
    ...
    torchinfo.summary(model, (2, 3, 346, 260))
